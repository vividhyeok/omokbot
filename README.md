...
- 세 번째 축으로 사용자 행동 예측 모델이 결합됩니다. 이전 수 전이(어디 다음에 어디를 두는지), 자주 만든 위협 좌표, 현재 국면의 즉시 위협을 합쳐 다음 사용자 착수 확률을 만들고 방어 우선순위를 조정합니다.

현재 루트 버전은 `/finite`, `/infinite` 분기 없이 하나의 메인 모델만 사용합니다.
...
#### 의사결정 근거 패널 (검증 가능)
사이드 패널에서 매 턴 다음 정보를 바로 확인할 수 있습니다.
- 수읽기 기여 %, 학습망 기여 %, 사용자예측 기여 %
- 봇이 선택한 수의 확률(후보 소프트맥스 기반)
- 사용자 다음 수 예측 Top3 좌표 및 확률
- 봇 후보수 Top3 좌표 및 확률

이 값들은 고정 텍스트가 아니라 실제 착수 순간 계산 결과를 그대로 표시합니다.
